...
const getVagueIntel = (army) => {
  // Simulates what a scout would see
...
export default function BattleSimulator() {
  ...
  const [phase, setPhase] = useState('menu'); // menu, setup, battle, report
  const [turn, setTurn] = useState(1);
  const [activePlayer, setActivePlayer] = useState('attacker'); // 'attacker' or 'defender'
  ...
  // Moves Buffer (for PvP hidden turns)
  ...
  const initGame = (selectedMode) => {
    ...
    const army1 = generateArmy(true); // Player 1
    const army2 = generateArmy(selectedMode === 'PvP'); // Player 2 or AI
    ...
    setPhase('battle');
    setActivePlayer('attacker'); // Attacker always starts
  };

  const processTurn = (atkMove, defMove) => {
    const newLogs = [];

    // 1. Analyze Moves (Simulated AI Engine)
    newLogs.push({ role: 'system', text: `--- TURN ${turn} RESOLUTION ---` });
    
    // Logic: Simple keyword matching to simulate strategy
    ...
    if (scenario.env.defenseBonus > 0 && defDefensive) {
      defDamage = Math.floor(defDamage * 0.5); // Defender takes less damage
    ...
    if (atkAggressive && defDefensive) {
      ...
      atkDamage += 100; // Attackers take more damage charging walls
    ...
    setActivePlayer('attacker'); // Reset to start of turn cycle
  };

  const handleCommand = () => {
    if (!inputBuffer.trim()) return;

    // 1. Commander/Officer Roleplay (Free Action)
    ...
    // 2. Game Move
    if (mode === 'PvE') {
      // Player is usually Human, Opponent is AI
  ...
  // Determine whose stats to show based on "Hotseat" hiding
  ...
  // If PvP and waiting for next player, show curtain
  const isHiddenState = mode === 'PvP' && phase === 'battle' && !logs[logs.length-1]?.text.includes('Resolution');
  
  // Which army is the "Player" currently?
  const currentArmy = activePlayer === 'attacker' ? attacker : defender;
  const enemyArmy = activePlayer === 'attacker' ? defender : attacker;
